...
export async function runNativeDensityChecks(cdp, delay) {
  ...
  const state = () =>
    ...
    const input = root.querySelector('#wall-day-density');
    const rect = e => {const r=e.getBoundingClientRect(); return {x:r.x,y:r.y,w:r.width,h:r.height};};
    return {button:rect(button),slider:rect(input),value:input.valueAsNumber,
  ...
  await press("Home", 36);
  assert((await state()).value === 40, "Native Home key did not zoom out.");
  await press("End", 35);
  assert((await state()).value === 96, "Native End key did not zoom in.");
  await press("ArrowLeft", 37);
  assert((await state()).value === 95, "Native Arrow key did not adjust density.");
  const { slider } = await state();
  const y = slider.y + slider.h / 2;
  await click(slider.x + slider.w / 4, y);
  assert((await state()).value < 64, "Native pointer cannot set compact density.");
  ...
  await delay(100);
  assert((await state()).value > 72, "Native touch drag cannot zoom toward more detail.");
